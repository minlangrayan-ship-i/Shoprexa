    ...
    <footer className="mt-20 border-t bg-dark text-slate-200">
      <div className="section grid gap-8 py-12 md:grid-cols-4">
        ...
        <div>
          ...
          <ul className="mt-3 space-y-2 text-sm">
            <li><Link href="/about">{t('A propos', 'About')}</Link></li>
        ...
        <div>
          ...
          <ul className="mt-3 space-y-2 text-sm">
            ...
            <li>{t('Base a Yaounde, Cameroun', 'Based in Yaounde, Cameroon')}</li>
          </ul>
        </div>
      </div>
      <p className="border-t border-white/10 py-4 text-center text-xs">{new Date().getFullYear()} Min-shop. {t('Tous droits reserves.', 'All rights reserved.')}</p>
    </footer>
  );
}
