...
export default function HomePage() {
  const { country, city, products, reviews, testimonials, sellers, users, t, isHydrated } = useSite();

  const featuredProducts = useMemo(() => {
    const regionalProducts = products.filter((product) => product.sellerCountry === country);
    const source = regionalProducts.length > 0 ? regionalProducts : products;
