...
import { marketplaceProducts, rankSellersByRating } from '@/lib/mock-marketplace';
import { useSite } from '@/components/site-context';

export default function ClientHomePage() {
  const { sessionUser, country, city, reviews, t } = useSite();
    ...
    return (
      <section className="section py-12">
        <div className="card p-6">
          ...
          <Link href="/auth/login" className="mt-4 inline-block rounded-lg bg-dark px-4 py-2 text-white">
            {t('Aller a la connexion', 'Go to login')}
          </Link>
        </div>
      </section>
    );
    ...
    <section className="section py-12">
      <div className="card bg-gradient-to-r from-brand-600 to-dark p-6 text-white">
        ...
        <p className="mt-2 text-sm opacity-90">
          {t('Nous te montrons les meilleurs produits pour', 'We are showing the best products for')} {city}, {country}.
        </p>
      </div>

      <div className="mt-8 grid gap-6 md:grid-cols-3">
        <div className="card p-4">
          <p className="text-sm text-slate-500">{t('Ville active', 'Active city')}</p>
          <p className="text-xl font-bold">{city}</p>
        </div>
        <div className="card p-4">
          <p className="text-sm text-slate-500">{t('Pays actif', 'Active country')}</p>
          <p className="text-xl font-bold">{country}</p>
        </div>
        <div className="card p-4">
          <p className="text-sm text-slate-500">{t('Top vendeur local', 'Top local seller')}</p>
          <p className="text-xl font-bold">{topSeller ? topSeller.company : '-'}</p>
        </div>
      </div>
